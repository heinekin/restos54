...
Ext.ux.grid.DataDrop = (function(){
    ...
    function dataDropped(e, el){
        ...
        if (nv !== '') {
            ...
            if(error > 1){
                champs += '</ul>';
                Ext.Msg.alert('Attention !', error+' champs n\'ont pas été chargés : '+champs+'<br>Les colonnes du tableau de destination ne correspondent pas aux données chargées');
            }
            else if(error == 1){
                champs += '</ul>';
                Ext.Msg.alert('Attention !', error+' champ n\'a pas été chargé : '+champs+'<br>Les colonnes du tableau de destination ne correspondent pas aux données chargées');
            }
        }
    }
