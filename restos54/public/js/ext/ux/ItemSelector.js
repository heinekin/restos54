...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    iconAllLeft:"left2.gif",
    iconAllRight:"right2.gif",
    iconTop:"top2.gif",
    ...
    onRender: function(ct, position){
        ...
        this.toStore.on('clear', this.valueChanged, this);
        ...
        this.iconAllLeft = this.imagePath + (this.iconAllLeft || 'left2.gif');
        this.iconAllRight = this.imagePath + (this.iconAllRight || 'right2.gif');
        ...
        this.addAllIcon = el.createChild({tag:'img', src:this.switchToFrom?this.iconAllLeft:this.iconAllRight, style:{cursor:'pointer', margin:'2px'}, alt:'all left'});
        el.createChild({tag: 'br'});
        ...
        el.createChild({tag: 'br'});
        this.removeAllIcon = el.createChild({tag:'img', src:this.switchToFrom?this.iconAllRight:this.iconAllLeft, style:{cursor:'pointer', margin:'2px'}, alt:'all right'});
        el.createChild({tag: 'br'});
        if (!this.toSortField) {
    ...
    allFromTo : function() {
        var records = this.fromMultiselect.view.store.data.items;
        
        this.toMultiselect.view.store.add(records);
        this.clearFrom();
    ...
    allToFrom : function() {
        var records = this.toMultiselect.view.store.data.items;

        this.fromMultiselect.view.store.add(records);
        this.clearTo();
    ...
    addFrom: function(data) {
        this.fromMultiselect.view.store.add(new Ext.data.Record(data));
    },

    clearFrom: function() {
        this.fromMultiselect.view.store.removeAll();
    },

    updateFrom: function(datas) {
        this.clearFrom();

        datas = $A(datas);
        for(var i = 0; i < datas.length; ++i)
        {
            this.addFrom(datas[i]);
        }
    },

    addTo: function(data) {
        this.toMultiselect.view.store.add(new Ext.data.Record(data));
    },
