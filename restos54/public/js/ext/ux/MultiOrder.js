...
/** 
 * @class Ext.ux.MultiOrder
 * @extends Ext.form.Field
 * A control that allows selection and form submission of multiple list items. The MultiOrder control
 * depends on the Ext.ux.DDView class to provide drag/drop capability both within the list and also 
 * between multiple MultiOrder controls (see the Ext.ux.ItemSelector).
 * 
 *  @history
 ...
 */
Ext.ux.Multiorder = Ext.extend(Ext.form.Field,  {
    /**
     ...
     */
    /**
     * @cfg {Ext.ux.DDView} view The Ext.ux.DDView used to render the multiorder list.
     */
    /**
    ...
    data:[],
    /**
     * @cfg {Number} width Width in pixels of the control (defaults to 'auto').
     */
    width:'auto',
    /**
     * @cfg {Number} height Height in pixels of the control (defaults to 'auto').
     */
    height:'auto',
    /**
    ...
    // false pour ne retourner que les valeurs sélectionnées, true pour renvoyer toutes les valeurs de facon ordonnée
    allValue: false,
    ...
    initComponent: function(){
        Ext.ux.Multiorder.superclass.initComponent.call(this);
    ...
    onRender: function(ct, position){
        Ext.ux.Multiorder.superclass.onRender.call(this, ct, position);
        var cls = 'ux-mselect';
        ...
        // raffraichit le champ texte caché a chaque modification des données de la liste
        ...
        //var hiddenTag={tag: "input", type: "text", value: "", name:this.name};
        if (this.isFormField) { 
    ...
    toTop : function() {
        ...
        if (selectionsArray.length > 0) {
            ...
            for (var i=0; i<selectionsArray.length; i++) {
                record = this.view.store.getAt(selectionsArray[i]);
                records.push(record);
            }
            selectionsArray = [];
            for (var i=records.length-1; i>-1; i--) {
                ...
                this.view.store.insert(0, record);
                selectionsArray.push(((records.length - 1) - i));
            }
        }
        this.view.refresh();
        this.view.select(selectionsArray);
    },
    ...
    toBottom : function() {
        ...
        if (selectionsArray.length > 0) {
            ...
            for (var i=0; i<records.length; i++) {
                ...
                selectionsArray.push((this.view.store.getCount()) - (records.length - i));
            }
        }
        this.view.refresh();
        this.view.select(selectionsArray);
        ...
        if (selectionsArray.length > 0) {
            for (var i=0; i<selectionsArray.length; i++) {
                ...
                if ((selectionsArray[i] - 1) >= 0) {
                    this.view.store.remove(record);
                    this.view.store.insert(selectionsArray[i] - 1, record);
                    newSelectionsArray.push(selectionsArray[i] - 1);
                }
            }
            this.view.refresh();
            this.view.select(newSelectionsArray);
        }
    },
    ...
    down : function() {
        ...
        if (selectionsArray.length > 0) {
            for (var i=0; i<selectionsArray.length; i++) {
                record = this.view.store.getAt(selectionsArray[i]);
                if ((selectionsArray[i] + 1) < this.view.store.getCount()) {
                    this.view.store.remove(record);
                    this.view.store.insert(selectionsArray[i] + 1, record);
                    newSelectionsArray.push(selectionsArray[i] + 1);
                }
    ...
    getValue: function(valueField){
        if(this.allValue) {
            ...
            return this.getSelectedValue(valueField);
        }
    },
     ...
     */
    getSelectedValue: function(valueField){
        var returnArray = [];
        var selectionsArray = this.view.getSelectedIndexes();
...
Ext.reg("multiorder", Ext.ux.Multiorder);
