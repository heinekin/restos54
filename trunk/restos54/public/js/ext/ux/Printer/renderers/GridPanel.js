...
Ext.ux.Printer.GridPanelRenderer = Ext.extend(Ext.ux.Printer.BaseRenderer, {
  ...
  generateBody: function(grid) {
    ...
    var date = new Date();
    return String.format('<table><tr><td>Restos du coeur - Vandoeuvre le ' + date.getDate() +'/'+ date.getMonth()+'/'+ date.getFullYear()+' </td></tr></table><table>{0}<tpl for=".">{1}</tpl></table>', headings, body);
  },
